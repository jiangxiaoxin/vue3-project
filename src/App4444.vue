<template>
  <div class="app-rooter">
    <div ref="divRef" v-collect:click.foo="123">多次添加</div>
    <div v-collect:bar="bar">
      <div>{{ bar.time }}</div>
      <HelloWorld :msg="bar.time" />
    </div>
    <button @click="changeBarTime">改</button>
    <div @click="oneClick" @dblclick="doubleClick">123123</div>
    <el-tag>111 </el-tag>
    <el-tag color="processing">2222 </el-tag>
    <el-tag color="red">333 </el-tag>
    <el-tag color="#00ff00">444 </el-tag>

    <div>
      <a-tag color="processing">111 </a-tag>
    </div>
...
onMounted(() => {
  divRef.value.addEventListener('click', afterClick)
  divRef.value.addEventListener('click', afterClick)
  divRef.value.addEventListener('click', afterClick)
})
